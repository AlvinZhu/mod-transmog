...
public:
    ...
    bool OnGossipHello(Player* player, Creature* creature) override
    {
        ...
        if (sT->GetEnableTransmogInfo())
            AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/INV_Misc_Book_11:30:30:-18:0|t怎样使用换化？", EQUIPMENT_SLOT_END + 9, 0);
...
#endif
        AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/INV_Enchant_Disenchant:30:30:-18:0|t移除所有幻化", EQUIPMENT_SLOT_END + 2, 0, "移除所有已装备物品的幻化？", 0, false);
        ...
        switch (sender)
        {
            ...
            case EQUIPMENT_SLOT_END + 4: // Presets menu
            {
                ...
                if (sT->GetEnableSetInfo())
                    AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/INV_Misc_Book_11:30:30:-18:0|t怎样使用套装？", EQUIPMENT_SLOT_END + 10, 0);
            ...
            case EQUIPMENT_SLOT_END + 6: // view preset
            {
                ...
                AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/INV_Misc_Statue_02:30:30:-18:0|tUse this set", EQUIPMENT_SLOT_END + 5, action, "使用这个套装进行幻化会将幻化的物品与你绑定在一起，并使它们不能被退还和交易。\n你想继续吗？\n\n" + sT->presetByName[player->GetGUID()][action], 0, false);
                AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "|TInterface/PaperDollInfoFrame/UI-GearManager-LeaveItem-Opaque:30:30:-18:0|t删除套装", EQUIPMENT_SLOT_END + 7, action, "Are you sure you want to delete " + sT->presetByName[player->GetGUID()][action] + "?", 0, false);
        ...
        if (oldItem)
        {
            ...
            if (sT->GetUseCollectionSystem())
            {
                ...
                if (sT->collectionCache.find(accountId) != sT->collectionCache.end())
                {
                    ...
                    std::string searchDisplayValue(hasSearchString ? searchStringIterator->second : "查找....");
                    // Offset values to add Search gossip item
                    if (pageNumber == 0)
                    {
                        if (hasSearchString)
                        {
                            AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, sT->GetItemIcon(30620, 30, 30, -18, 0) + "查找: " + searchDisplayValue, slot + 1, 0, "查找什么物品？", 0, true);
                        }
                        else
                        {
                            AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, sT->GetItemIcon(30620, 30, 30, -18, 0) + "查找....", slot + 1, 0, "查找什么物品？", 0, true);
                        }
                    }
                    else
                    {
                    ...
                    if (sT->GetAllowHiddenTransmog())
                    {
                        ...
                        else
                        {
                            ...
                            AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "隐藏部位", slot, UINT_MAX, "正在隐藏这个部位的装备。\n您想继续吗？\n\n" + lineEnd, 0, false);
                        }
                    }
                    ...
                    for (uint32 i = startValue; i <= endValue; i++)
                    {
                        ...
                        AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, sT->GetItemIcon(newItem->GetEntry(), 30, 30, -18, 0) + sT->GetItemLink(newItem, session), slot, newItem->GetEntry(), "使用这个物品进行幻化会将幻化的物品与你绑定在一起，并使它们不能被退还和交易。\n你想继续吗？\n\n" + sT->GetItemIcon(newItem->GetEntry(), 40, 40, -15, -10) + sT->GetItemLink(newItem, session) + lineEnd, price, false);
                    }
                }
                ...
                AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/INV_Enchant_Disenchant:30:30:-18:0|t移除幻化", EQUIPMENT_SLOT_END + 3, slot, "移除这个部位的幻化？", 0, false);
            ...
            else
            {
                ...
                for (uint8 i = INVENTORY_SLOT_ITEM_START; i < INVENTORY_SLOT_ITEM_END; ++i)
                {
                    ...
                    if (sT->GetFakeEntry(oldItem->GetGUID()) == newItem->GetEntry())
                        continue;
                    ++limit;
                    AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, sT->GetItemIcon(newItem->GetEntry(), 30, 30, -18, 0) + sT->GetItemLink(newItem, session), slot, newItem->GetGUID().GetCounter(), "使用这个物品进行幻化会将幻化的物品与你绑定在一起，并使它们不能被退还和交易。\n你想继续吗？\n\n" + sT->GetItemIcon(newItem->GetEntry(), 40, 40, -15, -10) + sT->GetItemLink(newItem, session) + lineEnd, price, false);
                }
...
private:
    ...
    void AddToDatabase(Player* player, ItemTemplate const* itemTemplate)
    {
        ...
        {
            if (showChatMessage)
                ChatHandler(player->GetSession()).PSendSysMessage( R"(|c%s|Hitem:%u:0:0:0:0:0:0:0:0|h[%s]|h|r 已被添加到您的幻化外观收藏中。)", itemQuality.c_str(), itemId, itemName.c_str());
